...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    collect.add_argument("--codes", nargs="+", default=[], help="只采指定 ts_code")
    collect.add_argument("--period", default=None, help="报告期 YYYYMMDD")
    collect.set_defaults(handler="collect")

    scores = subparsers.add_parser("scores", help="对最新采集 raw 评分")
    scores.add_argument("--codes", nargs="+", default=[], help="只评指定 ts_code")
    scores.add_argument("--date", default=None, help="产物日期戳 YYMMDD，默认沿用采集文件名")
    scores.set_defaults(handler="scores")

    screen = subparsers.add_parser("screen", help="按行业/评级筛选最新评分")
    screen.add_argument("--industry", default=None)
    screen.add_argument("--rating", default=None)
