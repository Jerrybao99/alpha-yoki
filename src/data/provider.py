...
class BaseFetcher(ABC):
    ...
    约束：
    - 业务代码只依赖本抽象，禁止直接 import 具体数据源 SDK。
    - 实现类负责限流 / 重试 / 缓存，本接口只声明能力。
    - 主键统一用 ts_code（如 "600000.SH"），与 Tushare 接口一致。
    """

    @abstractmethod
    def fetch_stock_list(self) -> list[StockInfo]:
        """读取全部 A 股上市公司清单（来源 stock_basic）。
    ...
    def fetch_financials(self, ts_code: str, period: str | None = None) -> StockFeatures | None:
        ...
        多个接口的真实字段，按 OUTPUT_COLUMNS 输出；计算型需求
        （股东权益比/主营利润率等）不在此采集，留给评分纯函数计算。
        period=None 表示取最新报告期。
