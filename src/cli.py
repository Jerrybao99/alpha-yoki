...
from src.llm.preferences import (
    load_preferences,
    preferences_path,
    save_preferences,
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    report.add_argument(
        ...
        default=None,
        help="LLM Provider；交互终端未指定时显示选择菜单",
    )
    report.add_argument(
        "--model",
        default=None,
        help="指定具体模型；未指定时优先使用上次保存的选择，再回退到模型菜单",
    )
    report.add_argument(
        "--no-save",
        dest="save",
...
def run_models(provider: str | None, *, settings: Settings | None = None) -> int:
    """打印模型目录、实际配置端点和默认型号。"""
...
def run_report(
    ...
    *,
    ...
    if csv_path is None:
        print("无评分 CSV，请先运行 full_scores.py", file=sys.stderr)
        return EXIT_DATA
    ...
    if provider is None:
        if saved_preference is not None:
            ...
            print(f"已应用上次选择：{selected_provider} / {selected_model}")
        elif interactive:
            try:
    ...
    if model is not None:
        ...
        pass  # selected_model 已来自上次保存的选择
    elif interactive:
        try:
    ...
    print(f"Provider: {client.spec.name.value}")
    print(f"Model: {client.spec.model}")
    print(_fallback_label(fallback_client))
    ...
    try:
        ...
        print(f"LLM 报告生成失败（{type(exc).__name__}）", file=sys.stderr)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "report":
        return run_report(args.provider, args.model, save=args.save)
    if args.command == "models":
        return run_models(args.provider)
    return EXIT_CONFIG
